import isArrayEqual from '@/global/isArrayEqual'

describe('isArrayEqual 方法测试', () => {
  test('基本类型数组内容相同应该相等', () => {
    const arr1 = [1, 2, 3]
    const arr2 = [1, 2, 3]
    ...
    const arr3 = ['a', 'b', 'c']
    const arr4 = ['a', 'b', 'c']
    expect(isArrayEqual(arr3, arr4)).toBe(true)

    const arr5 = [1, 'a', undefined, null]
    const arr6 = [1, 'a', undefined, null]
    expect(isArrayEqual(arr5, arr6)).toBe(true)
  })
  test('数组内容里不相同应该不相等', () => {
  ...
  test('传参不是数组，应该报错', () => {
    const arr1 = '';
    const arr2 = null;
    expect(() => isArrayEqual(arr1, arr2)).toThrow('参数不为数组，无法比较');
  })
})
